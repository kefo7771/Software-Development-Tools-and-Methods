...
# import modules necessary for URL to JSON import
# source: http://www.pythonforbeginners.com/code-snippets/parse-json-objects-in-python
import urllib2
import json

# enable debugging 
# source: https://docs.python.org/3/howto/webservers.html
import cgitb
cgitb.enable()
...
'''
print contents
# state capitals source: https://raw.githubusercontent.com/tdlm/fun-with-python/master/states_and_capitals.py
states_caps = {
...
# test: color all states red
# for key in states_caps:
# print ("$('#{0}').css('fill', 'red')".format(key))

# using google maps geocoding api
# source for format of query and return data: https://developers.google.com/maps/documentation/geocoding/intro
# source for how to read JSON response: http://stackoverflow.com/questions/13921910/python-urllib2-receive-json-response-from-url 
gmap_key =  "AIzaSyDMGGtjASsqKN7jhU6XwWjcuPjLV-JPnFU"
for key in states_caps:
 gmap_url = 'https://maps.googleapis.com/maps/api/geocode/json?address={0}{1}&key={2}'.format(states_caps[key]['state'].replace(" ", ""),states_caps[key]['capital'].replace(" ", ""), gmap_key)
 gmap_resource = urllib2.urlopen(gmap_url)
 gmap_data = json.load(gmap_resource)
 lat = gmap_data['results'][0]['geometry']['location']['lat']
 lon = gmap_data['results'][0]['geometry']['location']['lng'] 
 
 forecast_url = "https://api.forecast.io/forecast/fbbe153e22bc632f62dede15a458a276/{0},{1}".format(lat,lon)
